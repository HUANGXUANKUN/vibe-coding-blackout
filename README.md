...
5. **每次 `make install` 之后必须重新授权** —— macOS 的 TCC 按代码签名认 App，ad-hoc 签名的 cdhash 随二进制内容变，那条授权记录就钉死在旧二进制上了。重装后**复选框看着还是勾上的，但手势已经死了** —— 最容易被误当成 App bug 的现象。
   ...
   **注意：在开关里点「关再开」并不能刷新它**（实测无效，记录仍指向旧 cdhash）。要清掉重来：
   ...
   再打开辅助功能面板勾一次即可，不用重启 App —— 2 秒轮询会自动装上事件 tap。

   **彻底免除这个来回**：用一个固定的签名身份代替 ad-hoc。在钥匙串访问里「证书助理 ▸ 创建证书」建一个自签名的 Code Signing 证书（比如叫 `Blackout Local`），然后：

   ```bash
   CODESIGN_IDENTITY="Blackout Local" make install
   ```

   固定身份下 TCC 记的是证书而不是 cdhash，之后怎么重建都不会掉授权。有 Developer ID 的话同理。
