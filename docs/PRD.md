...
按**依赖**分组，而不是按数量。同一个依赖挂掉就一起挂掉的，不算两条路。

| 路径 | 依赖 | 何时可用 |
| --- | --- | --- |
| 再次双击触发键 | 事件 tap（辅助功能权限 + 无 secure input） | 主路径 |
| `esc` 被 tap 吞掉后触发恢复 | **同上** | 与主路径同生共死 |
| `esc` 走遮罩窗口自身的 `keyDown` | 无（纯 AppKit） | 遮罩取得 key focus 时 |
| 点击遮罩 | 无（纯 AppKit） | 手势不可用时 |
| 杀进程 | 无 | 永远 —— 遮罩随进程消失，亮度靠下次启动自愈 |

遮罩何时取 key focus：`手势不可用 || 输入拦截开启`。两种情况下按键本来就到不了前台 App，所以让遮罩接管键盘零成本；而这条路**不依赖任何权限**，是 G5 的真正兜底。遮罩是 `.nonactivatingPanel`，取 key focus 不会激活本 App，前台 App 仍是前台。
...
1. **黑屏时菜单栏被自己挡住** —— 遮罩在 shielding level。所以退出只能靠 §4.5 的那些路径，菜单里的 Restore 点不到。这是为了挡住通知横幅必须付的代价。
2. **Secure Event Input** —— 当前台是密码输入框（或开了 secure keyboard entry 的终端）时，macOS 会禁用所有事件 tap，手势失效。菜单里实时提示该状态；此时遮罩会自动接管键盘，`esc` 仍然有效。
...
## 第四轮 —— 实机验证发现（写完代码、装到 /Applications 之后）

前三轮是纸上评审。装机实跑后又抓到两个问题，其中一个是**前三轮判断错误**，值得单独记一笔。

| # | 问题 | 处置 |
| --- | --- | --- |
| 4.1 | **§4.5 声称的「三条独立路径」是错的。** 手势和 `esc` 都走同一个 CGEventTap —— 没授权辅助功能时 tap 装不上，两条一起失效；而遮罩在 shielding level 挡住了自己的菜单栏，菜单里的 Restore 也点不到。结果：**未授权时从菜单点一次 Black Out Now，用户就被锁死在黑屏里，只能去 Activity Monitor 杀进程**。这正是 G5 定义的灾难性失败，却被「路径数量 ≥ 3」这个假象掩盖了。 | 让遮罩窗口自己成为 key window 并处理 `keyDown` / `mouseDown` —— 纯 AppKit，**不依赖任何权限**。触发条件：`手势不可用 \|\| 输入拦截开启`（两种情况下按键本来就到不了前台 App，代价为零）。遮罩是 `.nonactivatingPanel`，取 key focus 不激活本 App。同时 §4.5 改为按**依赖**列表而不是按数量。另外：手势不可用时提示丸不再显示手势文案，改为 `press esc or click to restore` —— 否则等于叫用户去敲一个不会响的键。 |
| 4.2 | `--self-test` 在已安装的 App 正在运行时会失败，报「亮度未恢复」。真实原因是两个进程抢同一块屏的背光：自测恢复到 1.0 后 50ms，另一个实例又把它调到 0。错误信息把环境冲突伪装成了产品 bug。 | 自测启动时用 `NSRunningApplication` 检测同 bundle id 的其它实例，直接以 exit 2 拒绝运行并说明原因，不再产生误导性的失败。 |

顺带被实机数据证实的两点：

- **6.2 关于外接屏的判断是对的。** 本机 3 屏，`DisplayServicesGetBrightness` 只有内置 XDR（id=1）能读写，两台 DELL P3225QE 完全不响应。把「看不见」押在遮罩上而不是亮度 API 上，是这个产品能成立的前提。
- **6.4 崩溃一致性生效。** 黑屏中 `kill -9`，磁盘上留下 `{"1552-41055-4251086178-0": 1}`，下次启动自动还原并清除记录 —— A7 通过。
